...
def main():
    ...
    app.setApplicationName("Starframe")
    ...
    settings_mgr = SettingsManager("Starframe")
    ...
    if "--ship" in sys.argv:
        ...
        if s_idx < len(sys.argv):
            ship_to_deploy = sys.argv[s_idx]
            root_obj.selectShip(ship_to_deploy)
            root_obj.deploySelectedShip()

    if "--test-powerups" in sys.argv:
        root_obj.testPowerUps()

    if "--test-enemy-critical" in sys.argv:
        root_obj.testEnemyCritical()

    if "--test-paused-help" in sys.argv:
        root_obj.setProperty("splashEnabled", False)
        root_obj.togglePause()
        root_obj.openHelp()
    ...
    if "--screenshot" in sys.argv:
        root_obj.setProperty("splashEnabled", False)
        delay = 350
        if "--screenshot-delay" in sys.argv:
            d_idx = sys.argv.index("--screenshot-delay") + 1
            if d_idx < len(sys.argv):
                try:
                    delay = int(sys.argv[d_idx])
                except ValueError:
                    pass
        def capture():
