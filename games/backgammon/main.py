...
def main():
    ...
    if "--help" in sys.argv or "-h" in sys.argv:
        print("Omarchy Arcade • Backgammon (OA-026)")
        print("Usage: python main.py [--theme <theme_id>] [--list-themes]")
        sys.exit(0)
    ...
    app.setApplicationName("Backgammon")
    ...
    disk_candidates = [
        ...
        script_dir.parent.parent / "assets" / "covers" / "backgammon_disk.png",
        Path.home() / ".local" / "share" / "omarchy-arcade" / "assets" / "covers" / "backgammon_disk.png",
    ]
    ...
    settings_mgr = SettingsManager("Backgammon")
    ...
    if "--board-theme" in sys.argv:
        idx = sys.argv.index("--board-theme")
        if idx + 1 < len(sys.argv):
            b_theme = sys.argv[idx + 1]
            root_obj.setBoardTheme(b_theme)
    ...
    if "--screenshot-themes" in sys.argv:
        root_obj.setProperty("splashEnabled", False)
        def capture_themes():
            root_obj.setProperty("showThemePicker", True)
            out_path = Path(__file__).resolve().parent / "screenshot_themes.png"
            root_obj.captureScreenshot(str(out_path), False)
            QTimer.singleShot(400, app.quit)
        QTimer.singleShot(350, capture_themes)
