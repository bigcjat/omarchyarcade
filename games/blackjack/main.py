...
class SettingsManager(QObject):
    ...
    def setBankroll(self, amount):
        self.settings.setValue("bankroll", int(amount))
...
def main():
    ...
    if "--list-themes" in sys.argv:
        print("Blackjack 21 • Preset Themes:\n  --theme light\n  --theme dark\n  --theme <path_to_colors.toml>")
        sys.exit(0)

    app = QGuiApplication(sys.argv)
    app.setApplicationName("Blackjack 21")
    app.setOrganizationName("Arcade")
    script_dir = Path(__file__).resolve().parent
    disk_candidates = [
        script_dir / "assets" / "disk_icon.png",
        script_dir.parent.parent / "assets" / "covers" / "blackjack_disk.png",
        Path.home() / ".local" / "share" / "omarchy-arcade" / "assets" / "covers" / "blackjack_disk.png",
    ]
    ...
    settings_mgr = SettingsManager("Blackjack")
