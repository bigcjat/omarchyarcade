...
def main():
    ...
    app.setApplicationName("Arcade Game Template")
    ...
    settings_mgr = SettingsManager("GameTemplate")
    ...
    if theme_arg:
        ...
        if clean_arg in DEFAULT_PRESETS:
            ...
            if custom_path.is_file():
                ...
                print(f"Warning: Theme '{theme_arg}' not found. Using default.", file=sys.stderr)
        ...
        if system_colors:
            ...
            def on_theme_updated(path):
                ...
                if colors_path and colors_path.is_file():
                    if str(colors_path) not in watcher.files():
                        watcher.addPath(str(colors_path))
    ...
    if "--unmute" in sys.argv or "--sound" in sys.argv:
        root_obj.setProperty("isMuted", False)
    elif "--mute" in sys.argv:
        root_obj.setProperty("isMuted", True)
